...
1. `requisition = await _dbContext.Requisitions.AsNoTracking().Include(r => r.Stages.OrderBy(s => s.SortOrder)).FirstOrDefaultAsync(r => r.Id == requisitionId, ct)`
   — this single query is both the existence check and the ordered Stage fetch (NFR-1: query 1
   of 2; see Deviation Log — CP-2 originally shipped a separate `AnyAsync` existence check ahead
   of the Stages query, three round trips in total, before CP-4 collapsed the two). If
   `requisition == null` → `NotFound("requisition.pipeline.not-found", ...)`. `stages =
   requisition.Stages.ToList()`.
2. `apps = await _dbContext.Applications.AsNoTracking().Where(a => a.RequisitionId == requisitionId).Join(_dbContext.Users.AsNoTracking(), a => a.CandidateId, u => u.Id, (a, u) => new { a.Id, a.CurrentStageId, a.IsRejected, u.FirstName, u.LastName, Email = u.Email ?? "", a.SubmittedAtUtc }).ToListAsync(ct)`
   — query 2 of 2, then grouped **in memory** (NFR-1: ≤500 rows, trivial cost).
3. For each Stage (in order): `group = apps.Where(a => !a.IsRejected && a.CurrentStageId == stage.Id)` → `PipelineStageGroupDto(stage.Id, stage.Name, stage.SortOrder, group.Count, group.Select(ToBoardAppDto).ToList())`.
4. `rejected = apps.Where(a => a.IsRejected)` → `PipelineRejectedGroupDto(rejected.Count, rejected.Select(ToBoardAppDto).ToList())`.
5. Return `Ok(new PipelineBoardDto(requisitionId, stageGroups, rejectedGroup))`. Empty
   Requisition (0 Applications) still returns every Stage at zero count (AC-19), since step 3
...
| 2026-08-13 | T-54 (CP-4) | §3.2 `GetPipelineBoardAsync` | Exactly two `AsNoTracking()` queries (Stages; Applications joined to Users) | CP-2 shipped a third, separate `_dbContext.Requisitions.AsNoTracking().AnyAsync(...)` existence check ahead of the Stages query — three round trips, not two | NFR-1 verification (T-54) counted the commands actually issued and found three, not two, contradicting this LLD's own stated design and HLD §7's NFR-1 row. Fixed by replacing the separate existence check and Stages query with one `Requisitions.AsNoTracking().Include(r => r.Stages.OrderBy(...)).FirstOrDefaultAsync(...)` call, which is simultaneously the existence check (`null` → 404) and the ordered Stage fetch — collapsing three queries to two, matching the original design intent. `Requisition.Stages` was already a mapped navigation (`RequisitionConfiguration.HasMany(r => r.Stages)`), so no schema change was needed. |
