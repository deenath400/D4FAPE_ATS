...
| Create | `src/app/staff/requisitions/page.tsx` | Staff list (AC-12). Carries `export const dynamic = "force-dynamic"` — see Deviation Log |
...
| 2026-08-05 | T-25 | §1 File Manifest, `src/app/staff/requisitions/page.tsx` | No `export const dynamic` called out | Added `export const dynamic = "force-dynamic"` | `npm run build` tried to statically prerender this page at build time — it has no `searchParams` and no dynamic route segment, so Next.js 15 had nothing to auto-detect as request-dependent (unlike `/jobs`, which reads `searchParams`, and the `[id]` detail pages, which have unresolved dynamic segments). The prerender pass called `invokeBackend` with no backend reachable and no `API_BASE_URL` configured at build time, failing the build. Staff data is session-dependent and must never be statically cached across sessions regardless of build behaviour; the explicit export makes that intent build-time-enforced rather than an accident of routing shape. |
